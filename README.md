...
- **Repo alineado con `main`:** `npm run sync:pull` (o `npm run sync` si además querés push). Tras `git fetch origin`, **`npm run repo:branch-alignment`** imprime tabla de ramas vs `origin/main` (también workflow **Informe alineación ramas** en Actions).
