...
| **Red AINA (organizaciones / propiedades de red)** | Ver `docs/kiteprop-network-aina.md` | Con `KITEPROP_PROPERTIES_SOURCE=network` o `network_fallback_json`, **sí** alimenta el catálogo (`loadPublicCatalogFromNetwork`) y borradores de directorio (`organizationDrafts`, `kpnet:*` desde payload). Con `json` + `KITEPROP_MERGE_NETWORK_ORGANIZATIONS=1`, solo organizaciones vía `GET …/organizations`. Auditoría: `GET /api/test-kiteprop-network-audit` si `KITEPROP_NETWORK_AUDIT_ENABLED=1`. |
